...
class Pcd2ImageTransform:
	...
	"""
        ...
        self.transformation = pd.DataFrame({'x': x, 
                                            ...
                                            'x_img': x_img,
                                            'y_img': y_img,
                                            'r': r}).sort_values(['y_img', 'x_img']).set_index(['y_img', 'x_img'])         
        return self
...
#                 self.pcd = self.pcd.transform(transform)
        ...
        self.transformation = pd.DataFrame({'x': x, 
                                            ...
                                            'y_img': y_img,
                                            'r': r,
                                            'label': self.pcd_labels}).sort_values(['y_img', 'x_img']).set_index(['y_img', 'x_img'])     
        return self
    
    def transform_fast(self):
        """
        Input: 
...
#         return range_image, mask, labels
        return np.nan_to_num(np.stack((X, Y, Z, range_image), axis=-1))
...
#         return range_image, mask, labels
    ...
    def fit_transform_fast(self, xyz):
        self.fit_fast(xyz)
    ...
    def inverse_transform(self, labels):
        ...
        pred = pd.DataFrame({"y_img": coo.row, "x_img": coo.col, "pred": coo.data}).set_index(['y_img', 'x_img'])

        return self.transformation.join(pred)[['x', 'y', 'z', 'pred']].to_numpy()
